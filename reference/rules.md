...
- `rule_sd()` — a location/spread criterion, covering both the SD
  criterion and the median absolute deviation criterion.

- `rule_mad()` — a thin alias of
  `rule_sd(center = "median", scale = "mad")`.

- `rule_recursive()` — the sample-size-dependent criteria of van Selst
  and Jolicoeur (1994).
...
of exactly 180 ms — 180 ms is not *below* 180 ms. Note that `trimr` uses
...
When the spread statistic cannot be used — fewer than two observed
trials in a group, or zero spread — nothing is flagged. A rule that
cannot be evaluated must not remove data.
...
response time component — the one rule in the package that returns
something other than 0 and 1, and the reason
[`rt_screen()`](https://www.gfrischkorn.org/rtprep/reference/rt_screen.md)
...
property of the model rather than of either package — but it is a reason
to check `attr(x, "fits")$contaminant_prop` against what you expected
...
guess than a fast trial that is an error — something an RT-only mixture
...
- **It can order overlapping guesses better than response time alone.**
  Where contaminants fall inside the valid distribution's range — the
  case RT-only detection fails at — the joint posterior ranks them more
  accurately.
...
- It assumes contaminants respond at `chance`. Get `chance` wrong —
...
to apply several. Two further, experimental rules are unexported and
documented in
