...
#' processes. Which trials were replaced, and by which process, comes back with
#' the data. That is what a real data set cannot give you, and what scoring a
#' screen requires: sensitivity and specificity need labels.
#'
...
#'   single-boundary Wald accumulators with a shared bound, drawn exactly from
#'   the inverse Gaussian.
#' @param process Which contaminant process replaces the selected trials:
#'
#'   * `"leading_edge"` generates anticipations. Uniform in a band around the
#'     observed minimum of the *clean* trials in this call (see
...
#'   * `"delay"` generates delayed start-ups. A genuine trial plus a uniform
#'     shift of `delay_min` to `delay_max` seconds; the response, and so the
#'     intact accuracy, is kept.
#'   * `"informationless"` generates lapses of evidence quality. The trial is
#'     redrawn from the core with its evidence scaled by `lapse_prop` while the
#'     speed of processing is held fixed: `lapse_prop = 0` is a complete lapse
