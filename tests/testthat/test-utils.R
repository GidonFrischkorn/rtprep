...
test_that(".as_upper() accepts the documented codings", {
  expect_equal(.as_upper(c(1, 0, 1)), c(TRUE, FALSE, TRUE))
  expect_equal(.as_upper(c(TRUE, FALSE)), c(TRUE, FALSE))
  expect_equal(
    .as_upper(c("correct", "error", "upper", "lower")),
    c(TRUE, FALSE, TRUE, FALSE)
  )
  expect_equal(
    .as_upper(factor(c("Correct", "ERROR"))),
    c(TRUE, FALSE)
  )
  expect_equal(
    .as_upper(c("yes", "no", "hit", "miss")),
    c(TRUE, FALSE, TRUE, FALSE)
  )
})

test_that(".as_upper() keeps missing responses missing", {
  expect_equal(.as_upper(c(1, NA, 0)), c(TRUE, NA, FALSE))
  expect_equal(.as_upper(c("correct", NA)), c(TRUE, NA))
})
...
test_that(".as_upper() refuses codings it would silently misread", {
  ...
  expect_error(.as_upper(c(1, 2, 2, 1)), "coded 0/1")
  expect_error(.as_upper(c(-1, 1)), "coded 0/1")
  expect_error(
    .as_upper(c("maybe", "correct")), "Unrecognized response"
  )
  expect_error(.as_upper(list(1, 2)), "must be numeric, logical")
})

# --- .check_rt --------------------------------------------------------------

test_that(".check_rt() rejects what no rule could handle", {
  expect_error(.check_rt("a"), "must be a numeric vector")
  expect_error(.check_rt(matrix(c(0.3, 0.4), 1, 2)), "numeric vector")
  expect_error(.check_rt(numeric(0)), "has length 0")
  expect_error(.check_rt(c(0.3, 0)), "Non-positive")
  expect_error(.check_rt(c(0.3, -1)), "Non-positive")
  expect_error(.check_rt(c(0.3, Inf)), "Infinite")
  expect_warning(.check_rt(c(0.3, 500)), "seconds")
  expect_silent(.check_rt(c(0.3, NA, 0.4)))
})
...
test_that(".group_key() keeps cells apart when labels contain the separator", {
  ...
  key <- .group_key(list(c("a.b", "a"), c("c", "b.c")), 2)
...
test_that(".group_key() respects existing factor levels", {
  ...
  expect_equal(.group_key(g, 6)$labels, c("low", "med", "high"))
})

test_that(".group_key() propagates missing components", {
  key <- .group_key(list(c("a", NA, "a"), c("x", "x", "x")), 3)
  expect_true(is.na(key$id[2]))
  expect_false(any(is.na(key$id[-2])))
})

test_that(".group_key() rejects a malformed grouping", {
  expect_error(.group_key(1:3, 10), "same length as")
  expect_error(.group_key(list(), 3), "at least one")
  expect_error(
...
test_that("bounds are inclusive on both sides", {
  expect_equal(.prob_from_bounds(c(1, 2, 3), 1, 3), c(1, 1, 1))
  expect_equal(.prob_from_bounds(c(0.9, 3.1), 1, 3), c(0, 0))
  expect_equal(
    .reason_from_bounds(c(0.9, 1, 3, 3.1), 1, 3),
